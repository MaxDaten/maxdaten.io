...
- `grep -A10 'rssPostsQuery' src/lib/sanity/queries.ts | grep 'body\['` confirms body[] field in
  query </verify> <done>@portabletext/to-html installed, rssPostsQuery defined with body
  field</done> </task>
...
2. Replace allPostsQuery with rssPostsQuery (remove allPostsQuery import entirely)
...
- dc:creator tags present with author names
- `grep -v 'rssPostsQuery' src/routes/rss.xml/+server.ts | grep -c 'allPostsQuery'` returns 0
  (allPostsQuery fully replaced) </verify> <done>RSS feed renders full HTML body content with author
  attribution, allPostsQuery removed</done> </task>
