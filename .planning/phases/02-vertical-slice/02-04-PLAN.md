...
autonomous: false
env_vars_note:
    'Environment variables (PUBLIC_SANITY_PROJECT_ID, PUBLIC_SANITY_DATASET, SANITY_PREVIEW_SECRET)
    are configured in Vercel dashboard and local .env file, not committed to repository'
...
<task type="auto">
  ...
  <files>Local .env (not committed), Vercel dashboard configuration</files>
  <action>
...
Vercel production (via Vercel dashboard > Project Settings > Environment Variables):

1. Add PUBLIC_SANITY_PROJECT_ID (visible to browser - PUBLIC prefix)
2. Add PUBLIC_SANITY_DATASET (visible to browser)
3. Add SANITY_PREVIEW_SECRET (server-only, no PUBLIC prefix)

Note: These are dashboard configurations, not committed files. The files_modified frontmatter is
empty because no files are committed with secrets.
...
1. **SLCE-01** (Vercel adapter): Pre-existing - project already uses @sveltejs/adapter-vercel
2. **SLCE-02** (Fetch posts from Sanity): Migrated post loads from Sanity
3. **SLCE-03** (Fetch gems from Sanity): Deferred to Phase 3 (per CONTEXT.md decision)
