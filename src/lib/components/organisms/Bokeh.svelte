  ...
  @use "sass:list";
	...
	@for $i from 1 through $bokehCount {
		.parallax > circle:nth-child(#{$i}) {
			...
			--random-translate-x: #{math.random() * 100% - 25%};
			--random-translate-y: #{math.random() * 100% - 25%};
			--random-scale: #{0.1 + math.random() * 0.3};

			$depth: $i;
			transform: translate3d(var(--random-translate-x), var(--random-translate-y), $i * 1px)
				scale(var(--random-scale));

			$baseColor: list.nth($colorSet, math.random(list.length($colorSet)));
			$depthAdjustedColor: color.adjust($baseColor, $saturation: ($i * -1%));
			fill: $baseColor;
			stroke: color.adjust($baseColor, $lightness: 50%);
			opacity: $bokehBaseOpacity + math.random(1) * 0.01;
			filter: brightness($bokehBaseBrightness + math.random() * 150%) blur(6px * math.random() + 2px);
			transform-origin: (math.random() * 100% - 50%) (math.random() * 100% - 50%);
			animation-duration: $bokehBaseAnimationDuration + math.random(30) * 1s;
		}
	}
